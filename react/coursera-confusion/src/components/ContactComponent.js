...
class Contact extends Component {
	...
	validate = (firstname, lastname, telnum, email) => {
		...
		if( (this.state.touched.firstname && firstname.length < 3) ||
			...
			errors.firstname = 'First name should be between 3 and 10 chars';

		if( (this.state.touched.lastname && lastname.length < 3) ||
				(this.state.touched.lastname && lastname.length > 10)
		)
			errors.lastname = 'Last name should be between 3 and 10 chars';

		const reg = /^\d+$/;
		if(this.state.touched.telnum && !reg.test(telnum) )
			errors.telnum = 'Tel. Number should be letters';

		if(this.state.touched.email && email.split('').filter(x=> x === '@').length !== 1)
			errors.email = 'Email should have @ char';
