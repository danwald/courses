...
class Gemini:
    ...
    def get_book(self, symbol: str = 'BTCUSD') -> dict:
        ...
        return {
            'asks': [
                Ask(Decimal(ask['price']), Decimal(ask['amount']), datetime.fromtimestamp(int(ask['timestamp'])))
                for ask in data['asks']
            ],
            'bids': [
                Bid(Decimal(bid['price']), Decimal(bid['amount']), datetime.fromtimestamp(int(bid['timestamp'])))
...
class Kraken:
    ...
    def get_book(self, symbol: str = 'XBTUSD') -> dict:
        ...
        return {
            'asks': [
                Ask(Decimal(ask[0]), Decimal(ask[1]), datetime.fromtimestamp(int(ask[2])))
                for ask in data['result']['XXBTZUSD']['asks']
            ],
            'bids': [
                Bid(Decimal(bid[0]), Decimal(bid[1]), datetime.fromtimestamp(int(bid[2])))
...
class Coinbase:
    ...
    @staticmethod
    def build_jwt(request_method = "GET", request_host = "api.coinbase.com", request_path = "/api/v3/brokerage/product_book"):
    ...
    def get_book(self, symbol: str = 'BTC-USD') -> dict:
        data = requests.get(f'{self.url}?product_id={symbol}', headers={'Authorization': f'Bearer {self.build_jwt()}'}).json()
